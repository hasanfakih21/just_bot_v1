use crate::{attacks::RAYS, board::Board, types::{BitBoard, Move, Piece, Side, Square}};
...
impl Board {
    pub fn see(&self, m: Move, threshold: i32) -> bool {
        ...
        let diagonals = self.state.pieces[Piece::Bishop as usize] | self.state.pieces[Piece::Queen as usize];
        let orthogonals = self.state.pieces[Piece::Rook as usize] | self.state.pieces[Piece::Queen as usize];

        let king_rays = [RAYS[m.get_to() as usize][self.get_king_square(Side::White) as usize], RAYS[m.get_to() as usize][self.get_king_square(Side::Black) as usize]];
        ...
        loop {
            ...
            if attacker == Piece::King && !(attackers & self.state.occupancies[stm.other() as usize]).is_empty() {
                break
            }

            occupancies.clear_bit((self.state.pieces[attacker as usize] & our_attackers).least_sig_bit().unwrap());
            stm = stm.other();
